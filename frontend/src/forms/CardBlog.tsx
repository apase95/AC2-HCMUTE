...
export const CardBlog : React.FC<CardProps> = (props: CardProps) => {
  ...
  return (
        ...
        >
          <button className="w-full aspect-[16/9] rounded-lg overflow-hidden
            outline outline-[1px] outline-primary-dark/80 shadow-lg"
          >
            <img 
              src={props.logoURL || "/card-default.jpg"}
              alt="card"/>
          </button>
          ...
          <div className="w-full flex-between flex-row">
            <button 
              className="px-4 w-full leading-8 text-2xl text-left font-bold text-white line-clamp-1"
            >
                {props.title}
            </button>
            {isOwnerOrAdmin && (
          ...
          {props.subscription && <button 
            className="px-4 w-full text-sm text-left font-semibold text-white/60 line-clamp-2 break-all"
          >
            {props.subscription}
          </button>}
