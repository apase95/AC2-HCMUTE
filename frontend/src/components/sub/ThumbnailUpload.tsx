      ...
      {preview ? (
        <>
            ...
            <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity z-10">
                <span className="text-white font-medium flex items-center gap-2 mb-3">
                    <FaImage /> Change Image
                </span>
                <button 
                    onClick={handleRemove}
                    className="px-3 py-1 bg-red-600/80 hover:bg-red-600 text-white text-xs rounded-full flex items-center gap-1 transition-colors"
                >
                    <FaTrash /> Remove
                </button>
            </div>
